...
kp clusterbuilder save builder --tag ${IMAGE_PREFIX}builder --stack base --store default --order example/kpack-builder-cfg/kpack-builder-order.yaml
...
kapp deploy --yes -a example-sc -f <(ytt --ignore-unknown-comments -f example/app-operator/ -f example-values.yaml)
...
# Or, you can watch status, wait for "Unknown  MissingValueAtPath" to become "True   Ready", then hit Ctrl+C
